...
const filterProducts = () => {
  filteredProducts.value = products.value.filter((p) => {
    const matchName = filterName.value
      ? p.productName.includes(filterName.value)
      : true;
    const matchCategory = filterCategory.value
      ? p.categoryName === filterCategory.value
      : true;
...
const openEditDialog = (id) => {
  editingProductData.value = filteredProducts.value.find(
    (p) => p.productId === id
  );
...
const deleteProduct = async () => {
  await axios.delete(
    `http://localhost:8080/product/${deletingProductId.value}`
  );
